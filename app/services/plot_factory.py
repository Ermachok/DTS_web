...
def collect_time_data(prep_item, selected_idxs, combiscope_times, value_key, error_key, is_large_func):
...
def collect_pe_t_data(prep_item, selected_idxs, combiscope_times):
    ...
    for idx, t_time in zip(selected_idxs, combiscope_times):
        if idx < len(temps) and idx < len(dens):
            ...
            is_large = (p_val == 0 and (raw_err_T > 0 or raw_err_n > 0)) or (rel_p > 1.0)
...
def add_traces(fig, data, color, name, legend_group, mode, line_dash_good='solid', line_dash_bad='dash',
               marker_symbol_good='circle', marker_symbol_bad='x', marker_size_good=5, marker_size_bad=9,
               bad_line_width=2, showlegend_good=True, showlegend_bad=False, visible_good=True, visible_bad=False):
    good = data['good']
    bad = data['bad']
    if good[0]:
        fig.add_trace(go.Scatter(
            x=good[0], y=good[1],
            error_y=dict(type='data', array=good[2], arrayminus=good[3], visible=True),
            mode=mode,
            line=dict(color=color, dash=line_dash_good),
            marker=dict(symbol=marker_symbol_good, size=marker_size_good, color=color),
            name=name,
        ...
        if bad[0]:
            fig.add_trace(go.Scatter(
                x=bad[0], y=bad[1],
                error_y=dict(type='data', array=bad[2], arrayminus=bad[3], visible=True),
                mode=mode,
                line=dict(color=color, dash=line_dash_bad),
                marker=dict(symbol=marker_symbol_bad, size=marker_size_bad, line=dict(width=bad_line_width),
                            color=color),
                name=name, legendgroup=legend_group, showlegend=showlegend_bad, visible=visible_bad
            ))
    else:
        if bad[0]:
            fig.add_trace(go.Scatter(
                ...
                marker=dict(symbol=marker_symbol_bad, size=marker_size_bad, line=dict(width=bad_line_width),
                            color=color),
                name=name, legendgroup=legend_group, showlegend=showlegend_good, visible=visible_bad
            ))
...
def make_interactive_plots(fibers, combiscope_times) -> List[str]:
    ...
    for f_idx, item in enumerate(prepared_fibers):
        data = collect_time_data(item, selected_idxs, combiscope_times, "temperatures", "errors_T", is_large_temp)
        if data is None:
            continue
    ...
    fig_n_t = go.Figure()
    for f_idx, item in enumerate(prepared_fibers):
        data = collect_time_data(item, selected_idxs, combiscope_times, "density", "errors_n", is_large_dens)
        if data is None:
            continue
        color = PALETTE[f_idx % len(PALETTE)]
        base_name = f"{item['z_cm']:.1f} cm"
    ...
    for time_idx, (idx, time_val) in enumerate(zip(selected_idxs, combiscope_times)):
        data = collect_z_data(prepared_fibers, idx, "temperatures", "errors_T", is_large_temp)
    ...
    fig_n_z = go.Figure()
    for time_idx, (idx, time_val) in enumerate(zip(selected_idxs, combiscope_times)):
        data = collect_z_data(prepared_fibers, idx, "density", "errors_n", is_large_dens)
        color = PALETTE[time_idx % len(PALETTE)]
        time_name = f"{time_val:.1f} ms"
        legend_group = time_name
...
def plot_separatrix_to_html(sep_data: dict, interactive: bool = False) -> str:
    ...
    if sep_data.get("body"):
        fig.add_trace(go.Scatter(
            x=sep_data["body"]["R"], y=sep_data["body"]["Z"],
            mode="lines", name="body", line=dict(color="black")
        ))
    if sep_data.get("leg_1"):
        fig.add_trace(go.Scatter(
            x=sep_data["leg_1"]["R"], y=sep_data["leg_1"]["Z"],
            mode="lines", name="leg 1", line=dict(color="black")
        ))
    if sep_data.get("leg_2"):
        fig.add_trace(go.Scatter(
            x=sep_data["leg_2"]["R"], y=sep_data["leg_2"]["Z"],
            mode="lines", name="leg 2", line=dict(color="black")
        ))
    ...
    fig.update_xaxes(
        ...
        range=x_range, autorange=False, fixedrange=True
    )
    fig.update_yaxes(
        showgrid=True, gridcolor="#e6e6e6",
        range=y_range, autorange=False, fixedrange=True
    )
